...
df <- filter(df.hh, n_waves==3) %>%
  mutate(land_med = if_else(land_qtle %in% c(1,2), 1, 2),
         ...
         loans_taken_past_year = loans_taken_past_year/1000,
         lag_remitt = lag_remitt/1000,
         lag_loan = lag_loan/1000,
         lag_remitt_dum = if_else(lag_remitt > 0, 1,0),
         lag_loan_dum = if_else(lag_loan > 0, 1,0),
...
```{r}
inftransreg <- feols(inf_transfers ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
loanreg2 <- feols(loans_taken_past_year ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

infreg <- feols(net_informal ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

finreg <- feols(financial_assets ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

mods <- list(aidreg, ngoreg, infreg, finreg, remreg2, migreg, loanreg2)
...
```{r}
remreg2 <- feols(remittance_income ~ log(total_income) + lag_remitt | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2 <- feols(loans_taken_past_year ~ log(total_income) + lag_loan | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2pois <- fepois(remittance_income ~ log(total_income) + lag_remitt | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2pois <- fepois(loans_taken_past_year ~ log(total_income) + lag_loan | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2d <- feols(remittance_income ~ log(total_income) + lag_remitt_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2d <- feols(loans_taken_past_year ~ log(total_income) + lag_loan_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2dpois <- fepois(remittance_income ~ log(total_income) + lag_remitt_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2dpois <- fepois(loans_taken_past_year ~ log(total_income) + lag_loan_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2p <- feols(remittance_income > 0 ~ log(total_income) + lag_remitt | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2p <- feols(loans_taken_past_year > 0 ~ log(total_income) + lag_loan | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2pd <- feols(remittance_income > 0 ~ log(total_income) + lag_remitt_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2pd <- feols(loans_taken_past_year > 0 ~ log(total_income) + lag_loan_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2iv <- feols(remittance_income ~ log(total_income) | as.factor(hhid) |
                    lag_remitt ~ lag_rainfall_month_7 + lag_rainfall_month_8 + lag_rainfall_month_9 + lag_rainfall_month_10 +
                            lag_rainfall_month_11 + lag_rainfall_month_12 + lag_rainfall_month_1 + lag_rainfall_month_2 + lag_rainfall_month_3 +
                            lag_rainfall_month_4 + lag_rainfall_month_5 + lag_rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2iv <- feols(loans_taken_past_year ~ log(total_income) | as.factor(hhid) |
                     lag_loan ~ lag_rainfall_month_7 + lag_rainfall_month_8 + lag_rainfall_month_9 + lag_rainfall_month_10 +
                            lag_rainfall_month_11 + lag_rainfall_month_12 + lag_rainfall_month_1 + lag_rainfall_month_2 + lag_rainfall_month_3 +
                            lag_rainfall_month_4 + lag_rainfall_month_5 + lag_rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

mods <- list(remreg2, loanreg2, remreg2pois, loanreg2pois, remreg2p, loanreg2p, remreg2iv, loanreg2iv, remreg2d, loanreg2d, remreg2pd, loanreg2pd)

print(etable(mods))
```


### Cyclicality - Rainfall Instruments

```{r}
inftransreg <- feols(inf_transfers ~ 1 | as.factor(hhid) | 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

aidreg <- feols(quake_aid ~ 1 | as.factor(hhid) | 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

ngoreg <- feols(NGO_transfers ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

migreg <- feols(migrants_past_year ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2 <- feols(remittance_income ~ 1 | as.factor(hhid) | 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2 <- feols(loans_taken_past_year ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

infreg <- feols(net_informal ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

finreg <- feols(financial_assets ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
livereg1 <- feols(livestock_purchases ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

livereg2 <- feols(livestock_sales ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

landreg1 <- feols(land_purchased ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

landreg2 <- feols(land_sales ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

eqreg1 <- feols(equip_purchases ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

eqreg2 <- feols(equip_sales ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

bizreg1 <- feols(business_investment ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

bizreg2 <- feols(business_asset_sales ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

schoolreg <- feols(school_costs ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

jewelreg <- feols(jewelery ~ log(total_income) | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

mods <- list(livereg1, livereg2, landreg1, landreg2, bizreg1, bizreg2, eqreg1, eqreg2, schoolreg, jewelreg)
...
livereg1 <- feols(livestock_purchases ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

livereg2 <- feols(livestock_sales ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

landreg1 <- feols(land_purchased ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

landreg2 <- feols(land_sales ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

eqreg1 <- feols(equip_purchases ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

eqreg2 <- feols(equip_sales ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

bizreg1 <- feols(business_investment ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

bizreg2 <- feols(business_asset_sales ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

schoolreg <- feols(school_costs ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)

jewelreg <- feols(jewelery ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
```{r}
cpubtransregvdc <- feols(work_aid_wages ~ as.factor(land_med) | as.factor(vdc_yr),
                    data = df, weights = df$wt_hh, vcov = ~strata)

cpubtransregvdc2 <- feols(non_quake_aid ~ as.factor(land_med) | as.factor(vdc_yr),
                    data = df, weights = df$wt_hh, vcov = ~strata)

cinftransregvdc <- feols(inf_transfers ~ as.factor(land_med) | as.factor(vdc_yr),
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
cloanregvdc <- feols(loans_taken_past_year ~ as.factor(land_med) | as.factor(vdc_yr),
                    data = df, weights = df$wt_hh, vcov = ~strata)

mods <- list(caidregvdc, cpubtransregvdc, cpubtransregvdc2, cngoregvdc, cremregvdc, cmigregvdc, cloanregvdc)

print(etable(mods))
```

### Quake effects on income and labor supply

```{r}
qinc <- lm(log(total_income) ~ gorkha_loss_ever,
              data = df, weights = df$wt_hh, subset = df$wave==1)

qincfe <- lm(log(total_income) ~ gorkha_loss_ever + as.factor(vdc_code),
              data = df, weights = df$wt_hh, subset = df$wave==1)

qinciv2 <- ivreg(log(total_income) ~ gorkha_loss_ever | shake_pga,
              data = df, weights = df$wt_hh, subset = df$wave==1)

qinciv3 <- ivreg(log(total_income) ~ gorkha_loss_ever + as.factor(district)| shake_pga + as.factor(district),
              data = df, weights = df$wt_hh, subset = df$wave==1)

qinc4 <- ivreg(log(total_income) ~ gorkha_loss_ever + as.factor(district) + log(elevation) + log(distance_epicenter) | 
              shake_pga + as.factor(district) + log(elevation) + log(distance_epicenter),
              data = df, weights = df$wt_hh, subset = df$wave==1)

qlab <- lm(labor_supply ~ gorkha_loss_ever,
              data = df, weights = df$wt_hh, subset = df$wave==1)

qlabfe <- lm(labor_supply ~ gorkha_loss_ever + as.factor(vdc_code),
              data = df, weights = df$wt_hh, subset = df$wave==1)

qlabiv2 <- ivreg(labor_supply ~ gorkha_loss_ever | shake_pga,
              data = df, weights = df$wt_hh, subset = df$wave==1)

qlabiv3 <- ivreg(labor_supply ~ gorkha_loss_ever + as.factor(district)| shake_pga + as.factor(district),
              data = df, weights = df$wt_hh, subset = df$wave==1)

qlab4 <- ivreg(labor_supply ~ gorkha_loss_ever + as.factor(district) + log(elevation) + log(distance_epicenter)| 
              shake_pga + as.factor(district) + log(elevation) + log(distance_epicenter),
              data = df, weights = df$wt_hh, subset = df$wave==1)

mods <- list(qinc, qinciv2, qinciv3, qinc4, qlab, qlabiv2, qlabiv3, qlab4)

stargazer(mods, type = "text", style = "QJE", omit = c("district", "elevation", "distance_epicenter", "Constant"), 
          df = FALSE, omit.stat = c("adj.rsq", "ser"))
weighted.mean(df$labor_supply[df$wave==1], df$wt_hh[df$wave==1])
...
```{r}
dfsum <- df.hh[,c("consumption", "food_consumption", "total_income", "productive_assets",
              "home_value", "home_investment", "liq_savings", "tot_savings", "livestock_value",
              ...
              "NGO_transfers", "pub_transfers", "inf_transfers", "gorkha_loss_ever")]

xtabwts <- sumstats.wtd(dfsum, df.hh$wt_hh, d = 0, labs = c("Consumption", "Food Consumption", "Income", "Productive Assets", 
                              "Home Value", "Home Investment", "Liquid Savings", "Total Savings", "Livestock Value", "Loans Taken Past Year",
                              "Loans Made Past Year", "Remittance Income", "Household Members", "Connected Migrants", 
                              "Skipped Meal(%)", "Earthquake Aid", "Earthquake Aid(%)", "NGO Aid", 
                              "Public Transfers", "Informal Transfers", "Earthquake Losses"))
...
depreg1a <- feols(log(home_value + 1) ~ years_ago_built | as.factor(hhid) + as.factor(wave),
                 data = df, weights = df$wt_hh, vcov = ~strata)

depreg1b <- feols(home_value ~ years_ago_built | as.factor(hhid) + as.factor(wave),
                 data = df, weights = df$wt_hh, vcov = ~strata)

depreg2 <- feols(log(home_value) ~ years_ago_built + home_investment| as.factor(hhid) + as.factor(wave),
                 data = df, weights = df$wt_hh, vcov = ~strata)

depreg2a <- feols(log(home_value + 1) ~ years_ago_built + home_investment | as.factor(hhid) + as.factor(wave),
                 data = df, weights = df$wt_hh, vcov = ~strata)

depreg2b <- feols(home_value ~ years_ago_built + home_investment | as.factor(hhid) + as.factor(wave),
                 data = df, weights = df$wt_hh, vcov = ~strata)
...
depreg3a <- feols(log(home_value + 1) ~ years_ago_built + home_investment + 
                   wave + as.factor(district):wave | as.factor(hhid),
                 data = df, weights = df$wt_hh, vcov = ~strata)

depreg3b <- feols(home_value ~ years_ago_built + home_investment +
                   wave + as.factor(district):wave | as.factor(hhid),
                 data = df, weights = df$wt_hh, vcov = ~strata)

etable(depreg1, depreg1a, depreg1b, depreg2, depreg2a, depreg2b, depreg3, depreg3a, depreg3b)
```
...
ggplot(df.housing, aes(x = as.factor(received_aid), y = hstock)) + 
  geom_col(fill = "darkorchid1") + labs(x = "Received Aid", y = "Housing Stock/Avg Income") + 
  scale_x_discrete(labels = c("No", "Yes")) +
  theme_bw()

ggsave(paste(dir(getwd()), "/presentation_materials/hstock_aid.png", sep = ""), width = 12, height = 7)

ggplot(filter(df.hh, home_investment < 25000)) +
  geom_histogram(aes(x = home_investment), fill = "darkorchid1", bins = 25) +
  labs(x = "Housing Investment Amount", y = "Count") +
  theme_bw()

ggsave(paste(getwd(), "/presentation_materials/hinvest_hist.png", sep = ""), width = 12, height = 7)

mean(df.hh$home_investment < 25000, na.rm = TRUE)
df.hh %>% 
  group_by(hhid) %>%
...
```{r}
df.transfers <- df.hh %>% 
  summarize(loans_taken_past_year = weighted.mean(loans_taken_past_year, w = wt_hh, na.rm = TRUE),
...
df.transfers <- df.hh %>% 
  summarize(loans_taken = weighted.mean(loans_taken_past_year>0, w = wt_hh, na.rm = TRUE),
...
ggplot(df.hh, aes(x = log(liq_savings), y = log(tot_savings-liq_savings))) + 
  geom_point(aes(color = shake_pga)) +
  geom_smooth()

summary(lm(tot_savings ~ liq_savings + gorkha_loss_ever, data = df.hh))
```
