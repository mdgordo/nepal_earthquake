...
```
### Progressivity - Avg Consumption - VDC-YR FE
Obviously could be reverse causal, but within a village year - high consumption/income correlated with aid received

```{r}
cpubtransregvdc <- felm(log(pub_transfers + 1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cinftransregvdc <- felm(log(inf_transfers + 1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

caidregvdc <- felm(log(quake_aid + 1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cngoregvdc <- felm(log(NGO_transfers + 1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cmigregvdc <- felm(log(migrants_past_year+1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cremregvdc <- felm(log(remittance_income + 1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cloanregvdc <- felm(log(loans_taken_past_year+1) ~ log(avg_cons) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
```{r}
cpubtransregvdc <- felm(log(pub_transfers + 1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cinftransregvdc <- felm(log(inf_transfers + 1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

caidregvdc <- felm(log(quake_aid + 1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cngoregvdc <- felm(log(NGO_transfers + 1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cmigregvdc <- felm(log(migrants_past_year+1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cremregvdc <- felm(log(remittance_income + 1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cloanregvdc <- felm(log(loans_taken_past_year+1) ~ class5 | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
```{r}
cpubtransregvdc <- felm(log(pub_transfers + 1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cinftransregvdc <- felm(log(inf_transfers + 1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

caidregvdc <- felm(log(quake_aid + 1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cngoregvdc <- felm(log(NGO_transfers + 1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cmigregvdc <- felm(log(migrants_past_year+1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cremregvdc <- felm(log(remittance_income + 1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cloanregvdc <- felm(log(loans_taken_past_year+1) ~ high_caste | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
b0 <- optbw("food_consumption", b = "dist_2_seg13", df = df.hh, fuzzy = TRUE, dist.exclude = "none", wide = TRUE, k = "triangular")[1,3]

df.sample <- filter(df.hh, abs(dist_2_seg13) <= b0 & designation!="none")
