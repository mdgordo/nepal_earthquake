...
df <- filter(df.hh, n_waves==3) %>%
  mutate(land_med = if_else(land_qtle %in% c(1,2), 1, 2),
         ...
         lag_loans = lag_loans/1000,
         lag_remitt_dum = if_else(lag_remitt > 0, 1,0),
         lag_loan_dum = if_else(lag_loans > 0, 1,0),
...
loanreg2 <- feols(new_loans_taken ~ log(total_income) + lag_loans | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2pois <- fepois(remittance_income ~ log(total_income) + lag_remitt | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2pois <- fepois(new_loans_taken ~ log(total_income) + lag_loans | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2d <- feols(remittance_income ~ log(total_income) + lag_remitt_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2d <- feols(new_loans_taken ~ log(total_income) + lag_loan_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2dpois <- fepois(remittance_income ~ log(total_income) + lag_remitt_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2dpois <- fepois(new_loans_taken ~ log(total_income) + lag_loan_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2p <- feols(remittance_income > 0 ~ log(total_income) + lag_remitt | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2p <- feols(new_loans_taken > 0 ~ log(total_income) + lag_loans | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

remreg2pd <- feols(remittance_income > 0 ~ log(total_income) + lag_remitt_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)

loanreg2pd <- feols(new_loans_taken > 0 ~ log(total_income) + lag_loan_dum | as.factor(hhid),
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
loanreg2iv <- feols(new_loans_taken ~ log(total_income) | as.factor(hhid) |
                     lag_loans ~ lag_rainfall_month_7 + lag_rainfall_month_8 + lag_rainfall_month_9 + lag_rainfall_month_10 +
                            lag_rainfall_month_11 + lag_rainfall_month_12 + lag_rainfall_month_1 + lag_rainfall_month_2 + lag_rainfall_month_3 +
                            lag_rainfall_month_4 + lag_rainfall_month_5 + lag_rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
loanreg2 <- feols(new_loans_taken ~ 1 | as.factor(hhid)| 
                       log(total_income) ~ rainfall_month_7 + rainfall_month_8 + rainfall_month_9 + rainfall_month_10 + 
                            rainfall_month_11 + rainfall_month_12 + rainfall_month_1 + rainfall_month_2 + rainfall_month_3 + 
                            rainfall_month_4 + rainfall_month_5 + rainfall_month_6,
                    data = df, weights = df$wt_hh, vcov = ~strata)
...
```{r}
dfsum <- df.hh[,c("consumption", "food_consumption", "total_income", "productive_assets", "investments", "school_costs",
              "home_value", "home_investment", "years_ago_built", 
              "currentlyliving", "n_migrants", "femalehh", "cash_savings", "remittance_income", 
...
xtabwts <- sumstats.wtd(dfsum, df.hh$wt_hh, d = 0, labs = c("Consumption", "Food Consumption", "Income", "Productive Assets", 
                              "Investments", "School Costs", "Home Value", "Home Investment", "Age of House", 
                              "Household Members", "Connected Migrants", "Female Headed(%)", "Cash Savings",
