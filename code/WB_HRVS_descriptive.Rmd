...
Income doesnt include remittances/transfers/loans or inputs
...
livereg <- felm(log(productive_assets + 1) ~ log(income_gross) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

mods <- list(aidregvdc, pubtransregvdc, inftransregvdc, ngoregvdc, remregvdc, migregvdc, loanregvdc, livereg)
...
cliveregvdc <- felm(log(productive_assets+1) ~ log(consumption) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

mods <- list(caidregvdc, cpubtransregvdc, cinftransregvdc, cngoregvdc, cremregvdc, cmigregvdc, cloanregvdc, cliveregvdc)
...
```{r}
pdam1 <- felm(log(gorkha_loss_ever+1) ~ log(durables_stock+1)
                        | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam2 <- felm(log(gorkha_loss_ever+1) ~ n_rooms
                       | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam3 <- felm(log(gorkha_loss_ever+1) ~ iron_cement_roof
                       | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam4 <- felm(log(gorkha_loss_ever+1) ~ cement_walls
                        | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam5 <- felm(log(gorkha_loss_ever+1) ~ land_qtle
                        | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam6 <- felm(log(gorkha_loss_ever+1) ~ log(asset_stock+1)
                       | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam7 <- felm(log(gorkha_loss_ever+1) ~ log(productive_assets+1)
                       | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

pdam8 <- felm(log(gorkha_loss_ever+1) ~ log(consumption)
                       | as.factor(vdc) | 0 | strata,
                    data = df, weights = df$wt_hh[df$wave==1], subset = df$wave==1, na.action = na.omit)

mods <- list(pdam2, pdam3, pdam4, pdam5, pdam6, pdam7, pdam1, pdam8)
...
```{r}
paid1 <- felm(received_aid ~ log(gorkha_loss_ever+1)
                        | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

paid2 <- felm(received_aid ~ log(gorkha_loss_ever+1) + caste_recode
                       | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

paid3 <- felm(received_aid ~ log(gorkha_loss_ever+1) + femalehh
                        | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

paid4 <- felm(received_aid ~ log(gorkha_loss_ever+1) + land_qtle
                       | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

paid5 <- felm(received_aid ~ log(gorkha_loss_ever+1) + log(consumption)
                       | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

mods <- list(paid1, paid2, paid3, paid4, paid5)

stargazer(mods, type = "text", style = "QJE", df = FALSE, omit.stat = c("adj.rsq", "ser"),
          notes = c("All regressions include village-year fixed effects. Standard errors clustered at the ward."), notes.align = "l")

weighted.mean(df.hh$received_aid[df.hh$gorkha_loss_ever>0], df.hh$wt_hh[df.hh$gorkha_loss_ever>0], na.rm = TRUE)
weighted.mean(df.hh$received_aid[df.hh$gorkha_loss_ever==0], df.hh$wt_hh[df.hh$gorkha_loss_ever==0], na.rm = TRUE)
```
