...
df.hh <- filter(df.hh, !is.na(total_income) & food_consumption>0 & total_income>0. & non_durables>0)
...
mean(df.hh$home_value, na.rm = TRUE)/260000
median(df.hh$home_value, na.rm = TRUE)/260000
```
...
```{r}
risksharing_all <- feols(log(non_durables) ~ log(total_income) | as.factor(hhid) + as.factor(vdc_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_hc1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):caste_recode | as.factor(hhid) + as.factor(vdc_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_fh1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):femalehh | as.factor(hhid) + as.factor(vdc_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_l1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):as.factor(land_med) | as.factor(hhid) + as.factor(vdc_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_q1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):gorkha_hh | as.factor(hhid) + as.factor(vdc_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)
...
risksharing_all <- feols(log(non_durables) ~ log(total_income) | as.factor(hhid) + as.factor(caste_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_hc1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):caste_recode | as.factor(hhid) + as.factor(caste_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_fh1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):femalehh | as.factor(hhid) + as.factor(caste_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_l1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):as.factor(land_med) | as.factor(hhid) + as.factor(caste_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

risksharing_q1 <- feols(log(non_durables) ~ log(total_income) + log(total_income):gorkha_hh | as.factor(hhid) + as.factor(caste_yr),
                         data = df, weights = df$wt_hh, vcov = ~strata)

mods <- list(risksharing_all, risksharing_hc1, risksharing_fh1, risksharing_l1, risksharing_q1)

modelsummary(dvnames(mods), gof_omit = "IC|Adj|Within|RMSE", stars = TRUE, output = "latex")
```
...
```{r}
risksharing_all <- feols(log(non_durables) ~ 1 | as.factor(hhid) | 
...
risksharing_hc1 <- feols(log(non_durables) ~ 1 | as.factor(hhid) | 
...
risksharing_fh1 <- feols(log(non_durables) ~ 1 | as.factor(hhid) | 
...
risksharing_l1 <- feols(log(non_durables) ~ 1 | as.factor(hhid) | 
...
risksharing_q1 <- feols(log(non_durables) ~ 1 | as.factor(hhid) | 
...
modelsummary(dvnames(mods), stars = TRUE, gof_omit = "IC|Adj|Within|RMSE", output = "latex")
```
...
```{r}
dfsum <- df.hh[,c("non_durables", "food_consumption", "total_income", "productive_assets", "investments", "jewelery", "school_costs",
...
xtabwts <- xtabwts[,c(1:5,8)]
print(xtable(xtabwts), include.rownames = FALSE)
...
modelsummary(dvnames(mods), gof_omit = "IC|Within|Adj|RMSE|F|Lik", stars = TRUE, coef_omit = "vdc_code|district", 
            add_rows = data.frame(rbind(c("Fixed Effectes", "", "Village", "", "District", "District"),
                                        c("IV", "", "", "Shake Intensity", "Shake Intensity", "Shake Intensity"),
                                        c("Wave", rep("1", 5)))), output = "latex")
...
modelsummary(mods, gof_omit = "IC|Within|Adj|RMSE", stars = TRUE, coef_omit = "district", output = "latex")
```
