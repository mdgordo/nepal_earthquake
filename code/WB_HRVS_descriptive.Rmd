...
df <- filter(df.hh, !is.na(labor_income) & food_consumption>0 & labor_income>0)
```

#### Summary Stats

```{r}
dfsum <- df[,c("consumption", "food_consumption", "total_income", "productive_assets",
...
```{r}
risksharing_all <- felm(log(food_consumption) ~ log(labor_income) 
                        | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_hc1 <- felm(log(food_consumption) ~ log(labor_income) + log(labor_income):caste_recode
                       | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_fh1 <- felm(log(food_consumption) ~ log(labor_income) + log(labor_income):femalehh
                        | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_l1 <- felm(log(food_consumption) ~ log(labor_income) + log(labor_income):land_qtle
                       | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_q1 <- felm(log(food_consumption) ~ log(labor_income) + log(labor_income):gorkha_hh
                       | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
```{r}
pubtransreg <- felm(log(pub_transfers + 1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

inftransreg <- felm(log(inf_transfers + 1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

aidreg <- felm(log(quake_aid + 1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

ngoreg <- felm(log(NGO_transfers + 1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

migreg <- felm(log(migrants_past_year+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

remreg2 <- felm(log(remittance_income + 1) ~ log(labor_income) + log(lag_remitt + 1) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

loanreg2 <- felm(log(loans_taken_past_year + 1) ~ log(labor_income) + log(lag_loan + 1) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

livereg1 <- felm(log(livestock_purchases+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

livereg2 <- felm(log(livestock_sales+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

landreg1 <- felm(log(land_purchased+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

landreg2 <- felm(log(land_sales+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

eqreg1 <- felm(log(equip_purchases+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

eqreg2 <- felm(log(equip_sales+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

bizreg1 <- felm(log(business_investment+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

bizreg2 <- felm(log(business_asset_sales+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

schoolreg <- felm(log(school_costs+1) ~ log(labor_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

mods <- list(aidreg, pubtransreg, ngoreg, remreg2, migreg, loanreg2)

stargazer(mods, type = "text", style = "QJE", df = FALSE, omit.stat = c("adj.rsq", "ser"),
          add.lines = list(c("Fixed Effects", rep("Household", 6)),
                           c("Cluster", rep("Ward", 6))))

mods <- list(livereg1, livereg2, landreg1, landreg2, eqreg1, eqreg2, bizreg1, bizreg2, schoolreg)

stargazer(mods, type = "text", style = "QJE", df = FALSE, omit.stat = c("adj.rsq", "ser"),
          add.lines = list(c("Fixed Effects", rep("Household", 9)),
                           c("Cluster", rep("Ward", 9))))
```
