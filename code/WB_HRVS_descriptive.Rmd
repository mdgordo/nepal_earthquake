...
```{r}
risksharing_all <- felm(log(food_consumption) ~ log(total_income) 
                        | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_hc1 <- felm(log(food_consumption) ~ log(total_income) + log(total_income):caste_recode
                       | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_fh1 <- felm(log(food_consumption) ~ log(total_income) + log(total_income):femalehh
                        | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_l1 <- felm(log(food_consumption) ~ log(total_income) + log(total_income):as.factor(land_med)
                       | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

risksharing_q1 <- felm(log(food_consumption) ~ log(total_income) + log(total_income):gorkha_hh
                       | as.factor(hhid) + as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
risksharing_all <- felm(log(food_consumption) ~ log(total_income) 
                        | as.factor(hhid) + as.factor(caste_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
stargazer(mods, type = "latex", style = "QJE", df = FALSE, omit.stat = c("adj.rsq", "ser"),
          notes = c("All regressions include household and caste-year fixed effects. Standard errors clustered at the ward."), notes.align = "l")
...
```{r}
inftransreg <- felm(inf_transfers ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

aidreg <- felm(quake_aid ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

ngoreg <- felm(NGO_transfers ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

migreg <- felm(migrants_past_year ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

remreg2 <- felm(remittance_income ~ log(total_income) + lag_remitt | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

loanreg2 <- felm(loans_taken_past_year ~ log(total_income) + lag_loan | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

infreg <- felm(net_informal ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

finreg <- felm(financial_assets ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
livereg1 <- felm(livestock_purchases ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

livereg2 <- felm(livestock_sales ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

landreg1 <- felm(land_purchased ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

landreg2 <- felm(land_sales ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

eqreg1 <- felm(equip_purchases ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

eqreg2 <- felm(equip_sales ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

bizreg1 <- felm(business_investment ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

bizreg2 <- felm(business_asset_sales ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

schoolreg <- felm(school_costs ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

jewelreg <- felm(jewelery ~ log(total_income) | as.factor(hhid) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

mods <- list(livereg1, livereg2, landreg1, landreg2, bizreg1, bizreg2, eqreg1, eqreg2, schoolreg, jewelreg)

stargazer(mods, type = "latex", style = "QJE", df = FALSE, omit.stat = c("adj.rsq", "ser"),
          add.lines = list(c("Fixed Effects", rep("Household", 10)),
                           c("Cluster", rep("Ward", 10))))
```
### Progressivity - land value - VDC-YR FE

```{r}
cpubtransregvdc <- felm(work_aid_wages ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cpubtransregvdc2 <- felm(non_quake_aid ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cinftransregvdc <- felm(inf_transfers ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

caidregvdc <- felm(quake_aid ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cngoregvdc <- felm(NGO_transfers ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cmigregvdc <- felm(migrants_past_year ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cremregvdc <- felm(remittance_income ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)

cloanregvdc <- felm(loans_taken_past_year ~ as.factor(land_med) | as.factor(vdc_yr) | 0 | strata,
                    data = df, weights = df$wt_hh, na.action = na.omit)
...
```{r}
qinc <- ivreg(log(total_income) ~ gorkha_loss_ever,
...
qlab <- ivreg(labor_supply ~ gorkha_loss_ever,
              data = df, weights = df$wt_hh, subset = df$wave==1)
...
mods <- list(qinc, qinciv2, qinciv3, qlab, qlabiv2, qlabiv3)

stargazer(mods, type = "latex", style = "QJE", omit = c("district", "Constant"), df = FALSE, omit.stat = c("adj.rsq", "ser"))
