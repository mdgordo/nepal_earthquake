...
Choose vars 
```{r}
placebos <- c("elevation", "shake_pga", "gorkha_loss", "gorkha_loss_amt", "high_caste", "age_hh", "highest_ed", 
...
```{r, message=FALSE, warning=FALSE}
plotlist <- lapply(aidvars, plotvar, b = "dist_2_seg13", df = df.hh, h = 20)
plotlist
...
```{r}
plotlist <- lapply(numvars, plotvar, b = "dist_2_seg13", df = df.hh)
plotlist
...
```{r}
mainvars <- c("consumption", "food_consumption", "income_gross", "loan_payments", "remittance_income", "n_migrants")
...
ctlist <- lapply(cvarscutpts, cutvars, var = "income_gross")
df <- cbind(df.hh, do.call(cbind, ctlist))
