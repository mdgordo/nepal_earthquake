...
bws <- rbind(optbw("aid_cumulative_bin", b = "dist_2_seg13", df = df.hh, dist.exclude = "none", wide = TRUE, k = "triangular"),
      optbw("food_consumption", b = "dist_2_seg13", df = df.hh, fuzzy = TRUE, dist.exclude = "none", wide = TRUE, k = "triangular"))
...
rdstg13 <- lapply(aidvars, regout, b = "dist_2_seg13", df = df.hh, h = 15, b0 = 15, k = "triangular", vce = "hc1", 
                 dist.exclude = "none", wide = TRUE)
...
rdstg1 <- c(rdstg11, rdstg13, rdstg14, rdstg15, rdstg12)

rdgazer(rdstg1, dvlabs = rep(c("Received Aid", "Cumulative Aid"), 5), type = "text", 
        xlines = list(c("Kernel", rep("triangular", 8), rep("epanechnikov", 2)),
                      c("Polynomial", rep("linear", 4), rep("quadratic", 2), rep("linear", 4)),
                      c("5km Donut", rep("", 6), rep("X", 2), rep("", 2)),
...
```{r, warning=FALSE}
mainvars <- c("consumption", "food_consumption", "home_investment", "income_gross", "productive_assets", "loan_payments", "total_outlays_net", "n_migrants")
...
reg1 <- ivreg(food_consumption ~ aid_cumulative_bin + aid_cumulative_bin:log(gorkha_loss_amt+1) + aid_cumulative_bin:as.factor(land_qtle) + 
                dist_pos + dist_neg + shake_pga + as.factor(wave) + as.factor(land_qtle) + 
               as.factor(border_segment13) + dist_2_seg13:as.factor(border_segment13) +
                high_caste + class5 + poly(age_hh,2) + slope + elevation + log(gorkha_loss_amt+1) | 
          dist_14 + dist_14:log(gorkha_loss_amt+1) + dist_14:as.factor(land_qtle) + dist_pos + dist_neg + shake_pga + as.factor(wave) + 
            as.factor(land_qtle) + as.factor(border_segment13) + dist_2_seg13:as.factor(border_segment13) + 
            high_caste + class5 + poly(age_hh,2) + slope + elevation + log(gorkha_loss_amt+1),
      data = df.int, cluster = strata, weights = kwt, subset = abs(dist_2_seg13)<h0)

reg3 <- ivreg(home_investment ~ aid_cumulative_bin + aid_cumulative_bin:log(gorkha_loss_amt+1) + aid_cumulative_bin:as.factor(land_qtle) + 
                dist_pos + dist_neg + shake_pga + as.factor(wave) + as.factor(land_qtle) + 
               as.factor(border_segment13) + dist_2_seg13:as.factor(border_segment13) +
                high_caste + class5 + poly(age_hh,2) + slope + elevation + log(gorkha_loss_amt+1) | 
          dist_14 + dist_14:log(gorkha_loss_amt+1) + dist_14:as.factor(land_qtle) + dist_pos + dist_neg + shake_pga + as.factor(wave) + 
...
reg5 <- ivreg(remittance_income ~ aid_cumulative_bin + aid_cumulative_bin:log(gorkha_loss_amt+1) + aid_cumulative_bin:as.factor(land_qtle) + 
                dist_pos + dist_neg + shake_pga + as.factor(wave) + as.factor(land_qtle) + 
               as.factor(border_segment13) + dist_2_seg13:as.factor(border_segment13) +
                high_caste + class5 + poly(age_hh,2) + slope + elevation + log(gorkha_loss_amt+1) | 
          dist_14 + dist_14:log(gorkha_loss_amt+1) + dist_14:as.factor(land_qtle) + dist_pos + dist_neg + shake_pga + as.factor(wave) + 
...
stargazer(reg1, reg3, reg5, type = "text",
