...
```{r}
qplot("durables_consumption", filter(df.hh, designation!="none")) + 
  theme(text = element_text(size = 20))
ggsave(paste(rootdir, "/presentation_materials/qplot_assets.png", sep = ""), width = 10, height = 7)
```

```{r}
qplot("home_investment", filter(df.hh, designation!="none")) +
  theme(text = element_text(size = 20))
ggsave(paste(rootdir, "/presentation_materials/qplot_assets.png", sep = ""), width = 10, height = 7)
```
...
```{r}
mainvars <- c("consumption", "food_consumption", "home_investment", "income_gross", "productive_assets", "loan_payments", "remittance_income", "n_migrants")
...
r <- rdplot(y = df$resreg, x = df$dist_2_seg13, c = 0, weights = df$wt_hh, binselect = "qsmv", kernel = "triangular", 
              poly = FALSE, title = NULL, y.label = "Coefficient of Consumption Smoothing", x.label = "Distance to Border", hide = TRUE, span = 1)
r$rdplot
```
...
```{r, warning = FALSE}
cvarscutpts <- c(0, 50000, 100000, 500000, 1e6, 3e6 )
...
```{r, warning = FALSE}
cvarscutpts <- c(0, 5000, 10000, 100000, 300000)
...
```{r, warning = FALSE}
cvarscutpts <- c(0, 10000, 50000, 100000, 300000)
ctlist <- lapply(cvarscutpts, cutvars, var = "remittance_income", filter(df.hh, designation!="none"))
df <- cbind(filter(df.hh, designation!="none"), do.call(cbind, ctlist))
...
```{r, warning = FALSE}
cvarscutpts <- c(0, 10000, 50000, 100000, 200000)
ctlist <- lapply(cvarscutpts, cutvars, var = "loan_payments", filter(df.hh, designation!="none"))
df <- cbind(filter(df.hh, designation!="none"), do.call(cbind, ctlist))
