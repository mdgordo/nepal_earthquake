### from pracma package - using lower tolerance
  ...
  if (method == "linear") {
    for (k in 1:length(xp)) {
      if ( k %in% i0) {
        ...
        v[k] <- sum(solve(A, b, tol = 1e-30) * c(1, xp[k], yp[k], xp[k]*yp[k]))
      } 
    }
...
approxfun2 = function(xgrid, hgrid, w) {
  function(xp, yp) {
    xp[xp>max(xgrid)] = max(xgrid)
    yp[yp>max(hgrid)] = max(hgrid)
    interp2(x = xgrid, y = hgrid, Z = w, xp, yp, method = "linear")
  }
}
