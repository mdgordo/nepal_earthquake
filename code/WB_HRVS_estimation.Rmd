...
rdstg1 <- c(rdstg11, rdstg12, rdstg13, rdstg14, rdstg15, rdstg16, rdstg17, rdstg18)
...
rdgazer(rdstg1[c(1:8)], dvlabs = rep(c("Received Aid", "Aid Amount"), 4), type = "text", 
        xlines = list(c("Kernel", rep("triangular", 2),  rep("epanechnikov", 2), rep("triangular", 4)),
                      c("Exclude Less affected Dists.", rep("X", 8)),
                      c("5 km Donut", rep("", 8)),
                      c("Border", rep("Point", 8)),
                      c("Wards", sapply(rdstg1, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))))

rdgazer(rdstg1[c(9:16)],dvlabs = rep(c("Received Aid", "Aid Amount"), 3), type = "text", 
        xlines = list(c("Kernel", rep("triangular", 8)),
                      c("Exclude Less affected Dists.", rep(rep("X", 2), rep("", 2), 2)),
                      c("5 km Donut", rep("X", 2), rep("", 6)),
                      c("Border", rep("Point", 4), rep("Eastern", 2), rep("14 Dists", 2)),
                      c("Wards", sapply(rdstg1, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))))
...
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("alt bws")
rdmain <- lapply(mainvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 30, b0 = 30, vars = "opt",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(mainvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 80, b0 = 80, vars = "opt",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("including slight")
rdmain <- lapply(mainvars, regout, b = bvar, df = df.hh, fuzzy = fvar, vars = "opt", 
                 dist.exclude = c("none"), k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
...
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1", df = df.hh, fuzzy = fvar, vars = "opt", 
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("whole border")
rdmain <- lapply(mainvars, regout, b = "dist_2_14", df = df.hh, fuzzy = fvar, vars = "opt", 
                 dist.exclude = "all", k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
```
...
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("alt bws")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 30, b0 = 30, vars = "opt",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 80, b0 = 80, vars = "opt",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("including slight")
...
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("eastern border")
rdmain <- lapply(secvars, regout, b = "dist_2_seg1", df = df.hh, fuzzy = fvar, vars = "opt", 
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("whole border")
rdmain <- lapply(secvars, regout, b = "dist_2_14", df = df.hh, fuzzy = fvar, vars = "opt", 
                 dist.exclude = "all", k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
```
