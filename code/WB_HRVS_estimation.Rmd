...
infravars <- c("time_to_school", "time_to_health", "time_to_market")
...
mainvars <- c("consumption", "food_consumption", "home_investment", "total_income", "productive_assets",
              "remittance_income", "loan_payments", "loans_taken_past_year", "migration_bin", "financial_assets")
...
plotvar(fvar, b = bvar, df = df.hh, dist.exclude = dist.exclude, p = "loess") + 
  labs(x = "Distance to Border", y = "Aid (1,000 NPR)") + theme(text = element_text(size = 20))
...
plotvar(fvar, b = bvar, df = df.hh, dist.exclude = dist.exclude, p = 1, h = h0, residualizer = TRUE, vars = ctrls1) + 
  geom_vline(aes(xintercept = -h0), color = "red") + geom_vline(aes(xintercept = h0), color = "red") + 
  labs(x = "Distance to Border", y = "Residualized Aid (1,000 NPR)") + theme(text = element_text(size = 20))
...
plotvar("pub_transfers", b = bvar, df = df.hh, dist.exclude = dist.exclude, p = "loess") + 
...
plotvar("productive_assets", b = bvar, df = df.hh, dist.exclude = dist.exclude, p = "loess") + 
...
rdgazer(rdstg1[c(1:8)], dvlabs = rep(c("Received Aid", "Aid Amount"), 4), type = "latex", 
...
rdgazer(rdstg1[c(9:14)],dvlabs = rep(c("Received Aid", "Aid Amount"), 3), type = "latex", 
