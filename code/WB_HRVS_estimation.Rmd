...
df.hh <- filter(df.hh, abs(dist_2_seg1pt)<150) %>%
...
rdstg19 <- lapply(aidvars[1:2], regout, b = "dist_2_seg1pt", df = df.hh, vars = "none", 
                 dist.exclude = dist.exclude, k = k, vce = vce)

rdstg110 <- lapply(aidvars[1:2], regout, b = "dist_2_seg1pt1", df = df.hh, vars = "none", 
                 dist.exclude = dist.exclude, k = k, vce = vce) ### can't calculate bandwidth with the none vdc included

rdstg111 <- lapply(aidvars[1:2], regout, b = "dist_2_seg1pt2", df = df.hh, vars = "none", 
                 dist.exclude = dist.exclude, k = k, vce = vce)

rdstg1 <- c(rdstg11, rdstg12, rdstg12a, rdstg13, rdstg14, rdstg15, rdstg16, rdstg17, rdstg18, rdstg19, rdstg110, rdstg111)

rdgazer(rdstg1[c(1:12)], dvlabs = rep(c("Received Aid", "Aid Amount"), 6), type = "text", 
        xlines = list(c("Kernel", rep("triangular", 2),  rep("epanechnikov", 2),  rep("uniform", 2), rep("triangular", 6)),
                      c("Include unaffected Dists.", rep("", 12)),
                      c("5 km Donut", rep("", 12)),
                      c("Border", rep("Eastern", 12)),
                      c("Wards", sapply(rdstg1[c(1:12)], function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))))

rdgazer(rdstg1[c(13:24)], dvlabs = rep(c("Received Aid", "Aid Amount"), 6), type = "text", 
        xlines = list(c("Kernel", rep("triangular", 12)),
                      c("Include unaffected Dists.", rep("", 2), rep("X", 2), rep("", 8)),
                      c("5 km Donut", rep("X", 2), rep("", 10)),
                      c("Border", rep("Eastern", 4), rep("Whole", 2), rep("Point 1", 2), rep("Point 2", 2), rep("Point 3", 2)),
...
rdplacebo <- lapply(placebos, regout, b = "dist_2_seg1pt", f = fvar, df = df.hh, h = 20, b0 = 20, 
                    dist.exclude = dist.exclude, vars = "none", k = k, vce = vce)

rdgazer(rdplacebo, dvlabs = str_replace_all(placebos, "_", " "), 
        xlines = list(c("Wards", sapply(rdplacebo, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")

rdplacebo <- lapply(placebos, regout, b = "dist_2_seg1pt", f = fvar, df = df.hh, h = 30, b0 = 30, 
                    dist.exclude = dist.exclude, vars = "none", k = k, vce = vce)
...
print("w controls")
rdmain <- lapply(mainvars, regout, b = bvar, df = df.hh, fuzzy = fvar, vars = "shake_pga", 
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
...
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("w controls")
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, vars = "shake_pga", 
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, vars = "opt", 
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("logged none")
...
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("alt bws")
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, h = 20, b0 = 20, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, h = 30, b0 = 30, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, h = 40, b0 = 40, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
...
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(mainvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, h = 60, b0 = 60, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(mainvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("including unaffected dists")
...
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("w controls")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, vars = "shake_pga", 
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
...
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("alt bws")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 20, b0 = 20, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
...
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 40, b0 = 40, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 50, b0 = 50, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, h = 60, b0 = 60, vars = "none",
                 dist.exclude = dist.exclude, k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("excluding unaffected dists")
rdmain <- lapply(secvars, regout, b = bvar, df = df.hh, fuzzy = fvar, vars = "none", 
                 dist.exclude = "none", k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("whole border")
...
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("point 1")
rdmain <- lapply(secvars, regout, b = "dist_2_seg1pt", df = df.hh, fuzzy = fvar, vars = "none", 
                 dist.exclude = "all", k = k, vce = vce, ihs = FALSE)
rdgazer(rdmain, dvlabs = str_replace_all(secvars, "_", " "), 
        xlines = list(c("Wards", sapply(rdmain, function(x) length(unique(x$X[abs(x$X)<x$bws[2,1]]))))), type = "text")
print("point 2")
